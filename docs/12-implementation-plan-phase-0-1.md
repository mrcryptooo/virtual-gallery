...
**Scope:** zod schemas in `packages/engine/domain/manifest/` for the full hierarchy (`Project → Building → Floor → Room → Panorama`, hotspots, views, tiles metadata, `formatVersion`); hierarchy indexes (panorama ⇄ building/floor/room lookups); `scripts/validate-packages.mjs` running every ADR-010 gate (schema, file existence, hotspot resolution, required text, link-graph connectivity, id uniqueness); a **hand-built fixture package** (2 buildings, 3 floors, 6 panoramas incl. one cross-floor link) with placeholder-colored tiles/previews/posters at correct sizes and paths — the permanent test asset for everything downstream.
